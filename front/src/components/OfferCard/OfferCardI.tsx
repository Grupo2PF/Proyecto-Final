...
export default function OfferCardI(props: any): JSX.Element {
  ...
  const handleFavs = (e: any) => {
    if(auth.currentUser){
       if(fav.some((el:any)=>el.offers === props.offers || fav.some((el:any)=>el.price === props.price)) && fav.some((el:any)=>el.transfers.length === props.transfers.length) && fav.some((el:any)=>el.userId === auth.currentUser?.uid)){
        swal(errorMessage)
       }
        else{
    const info = {
      ...
      <div className={styles.offersCard}>
        <div className={styles.offersCardMainInfo}>
          ...
          <div className={styles.offersCardButtons}>
            ...
            <button value={props} onClick={handleFavs}><AiOutlineStar/> Añadir a favs</button>
            <button
